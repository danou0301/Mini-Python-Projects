from ex10 import Record,Diagnoser,Node,build_tree,optimal_tree
...
def tree_check(tree1,tree2):
    ...
    if tree1.is_leaf() or tree2.is_leaf():
        if tree1.is_leaf() and tree2.is_leaf():
            return True
        return False
    if not tree_check(tree1.get_positive(),tree2.get_positive()):
        return False
    if not tree_check(tree1.get_negative(),tree2.get_negative()):
        return False
    return True

def print_tree(tree):
    if tree.get_negative() != None:
        n = print_tree(tree.get_negative())
    else:
        n = ""
    if tree.get_positive() != None:
        p = print_tree(tree.get_positive())
    else:
        p = ""
    return str(p)+" "+str(tree.get_data())+" "+str(n)
...
if __name__ == "__main__":
    ...
    error=diagnoser.calculate_error_rate(records)
    check(2,4.0/9,error,"")

    #test 3
    expected=["cold","healthy","influenza","dead"]
    result=diagnoser.all_illnesses()
    check(3, expected, result,None,list_check)

    #test 4
    expected="cold"
    result=diagnoser.most_common_illness(records)
    ...
    if not tree_check(result,root):
        print("expected: "+print_tree(root))
        print("result: "+print_tree(result))
    ...
    if not tree_check(result,new_root):
        print("expected: "+print_tree(new_root))
        print("result: "+print_tree(result))
